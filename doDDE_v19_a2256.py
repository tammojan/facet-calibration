...
def create_phaseshift_parset(msin, msout, source, direction, imsize, dynamicrange, freqstep=30):
...
if __name__ == "__main__":
    ...
    for source in do_sources:
        ...
        ## FIXME -- hard-wired CPU limit in what follows
            ...
            for ms_id, ms in enumerate(mslist):
                parset = create_phaseshift_parset(ms,
                                                msavglist[ms_id],
                                                source,
                                                directions[source_id],
                                                imsizes[source_id],
        ...
        if StartAtStep in ['preSC', 'doSC']:
            ...
            os.system('python '+SCRIPTPATH+'/selfcalv19_ww_cep3.py ' +
                      inputmslist + ' ' + source + ' ' + atrous_do[source_id] +
                      ' ' + str(imsizes[source_id]) + ' ' + str(nterms) + ' ' +
                      str(cellsizetime_a[source_id]) + ' ' +
                      str(cellsizetime_p[source_id]) + ' ' + TEC + ' ' +
                      clock + ' ' + str(dynamicrange[source_id]) + ' ' +
